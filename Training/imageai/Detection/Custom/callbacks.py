...
class Log(QObject, TensorBoard):
    
    batch = Signal(dict)
    epoch = Signal(dict)
    on_start = Signal(bool)
    on_stop = Signal(bool)
    info = Signal(dict)
    ...
    def on_train_begin(self, logs):
        ...
        self.info.emit(self.params)
        
        super().on_train_begin(logs)
        
    def on_batch_end(self, batch, logs=None):
        
        logs['loss'] = logs['yolo_layer_1_loss'] + logs['yolo_layer_2_loss'] + logs['yolo_layer_3_loss']
        
        self.batch.emit(logs)
